  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-zinc-900 overflow-y-scroll">
      ...
      <header className="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-black border-b border-zinc-800 relative md:gap-0 gap-4">
        {/* Logo with onClick handler to refresh the page */}
        <img
          src="src/steez.png"
          alt="STEEZ.GR Logo"
          className="h-20 w-auto scale-[3] origin-left cursor-pointer z-10" // Add z-10 to ensure it's above other elements          onClick={() => window.location.reload()} // Refresh the page on logo click
          onClick={() => {
          ...
          {cartItems.length === 0 ? (
            ...
            <ul className="space-y-2">
              {cartItems.map((item, index) => (
                <li key={index} className="flex gap-4 items-center border-b border-gray-200 pb-2">
                  {/* Product Image */}
                  <img
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    <button onClick={(e) => handleQtyChange(index, -1, e)} className="px-2 py-1 bg-gray-200 rounded hover:bg-gray-300 transition">-</button>
                    <button onClick={(e) => handleQtyChange(index, 1, e)} className="px-2 py-1 bg-gray-200 rounded">+</button>
      ...
      {(activeSection === "Αναζήτηση Δέματος" || activeSection === "Αρχική Σελίδα") && (
        <section className="py-20 px-4 md:px-16 bg-black text-center"> {/* Changed bg-gray-800 to bg-black */}
        <div className="max-w-3xl mx-auto">
